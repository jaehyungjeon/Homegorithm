...
/*
	Question : 미국으로 유학간 동혁이는 세탁소를 운영하고 있다. 동혁이는 최근에 아르바이트로 고등학생 리암을 채용했다.
	...
	Solution : 1. 나머지 연산을 진행한 뒤 배열을 담는 리스트에 값을 넣어 도출하였다.
*/
...
public class LaundryCEO {
	...
	public static void main(String[] args) throws Exception {
		...
		for(int i=0; i<N; i++) {
			...
			int quarter = (value / arr[0]);
			value %= arr[0];
			
			int dime = (value / arr[1]);
			value %= arr[1];
			
			int nickel = (value / arr[2]);
			value %= arr[2];
			
			list.add(new int[]{quarter, dime, nickel, value});
		}
