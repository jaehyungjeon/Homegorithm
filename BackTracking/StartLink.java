...
/*
	...
	Solution : 1. 같은 위치를 방문하지 않기 위해 (1-1, 2-2.. 제외) for문을 넘겨주는 idx의 +1값으로 돌려줌
			   2. depth가 N/2 일 때 즉, 대각선을 기점으로 절반만 돌렸을 때를 기준으로 함.
			   3. 시작의 합은 Anm 기준 n과 m을 모두 방문했을때를 기점으로 갱신해주고 링크는 해당 지점을 제외한 나머지의 합으로 규정.
			   4. Min값은 start와 link의 차를 절댓값으로 구한다음 리턴해주면 됨.
			   5. 만약 둘의 최소가 0일 경우 더이상 최적의 조건이 없으므로 바로 출력해준 뒤, 프로그램을 종료함.
